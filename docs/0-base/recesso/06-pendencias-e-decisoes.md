...
| ~~**R-03**~~ | ~~PJ com mais de um contrato: qual ancora o período?~~ | ⛔ **Superada (2026-07-27):** um PJ **nunca** presta serviço a duas empresas ao mesmo tempo — sempre rescinde e gera novo contrato. O direito de recesso é **sempre 100%** do contrato ativo. Proporção removida. Ver `02` §1 |
| **R-05** | **Débito** pode deixar o saldo **negativo**? | **Bloquear** (422 no backend) | Permitir muda validação e o VO `SaldoDeDias` |
| ~~**R-08**~~ | ~~Contrato encerrado no **meio** de um mês: gera crédito **proporcional**?~~ | ✅ **Resolvida (2026-07-22 / atualizada 2026-07-27):** regra dos **15 dias** — ≥15 dias desde o último cálculo gera `2,5`; abaixo disso, zero. Proporção removida (sempre 100%). Encerramento manual eliminado — status deriva da vigência. Ver `02` §4 |
| ~~**R-17**~~ | ~~Σ das **proporções** dos contratos de um PJ~~ | ⛔ **Superada (2026-07-27):** como um PJ nunca tem dois contratos ativos simultâneos, a proporção é sempre 100% e não precisa de validação. Campo `proporcaoDeRecesso` removido do contrato |
...
|---|---|---|
| **R-04** | **Identidade do usuário** para "quem lançou" | ✅ **Resolvida (A-28 + A-29):** o app será uma **SPFx web part** no SharePoint; a identidade vem do **Entra ID** via `AadHttpClient` e o `lancado_por` é derivado do **token validado na CITY API** — nunca do payload. Ver [`docs/spfx-sharepoint/`](../../5-deploy-producao/spfx-sharepoint/README.md). Fase 1: usuário fixo via port `UsuarioAtual`. Resta a **autorização** (**S-06**) |
...
| **R-13** | Filtros **Mês** e **Ano** no HUD do recesso | **Mês não se aplica** (a tela não filtra por mês); Ano: opcional |
...
| **R-18** | Ordenação do extrato por outras colunas | Manter **cronológica**; se permitir outra, **ocultar a coluna Saldo** (saldo acumulado fora de ordem não tem significado) |

## 4. Como sinalizar no código

```ts
// PENDÊNCIA R-16: responsavelLegal não vem do HCM hoje — mock na Fase 1.
const responsavelLegal = fornecedor.responsavelLegal ?? fornecedor.apelido;
```
